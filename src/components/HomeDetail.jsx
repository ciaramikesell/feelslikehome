...
import { ArrowLeft, ExternalLink, Heart, Footprints, Home as HomeIcon, Pencil, RotateCcw, Star } from 'lucide-react';
  ...
  return <main className="hh-home-detail" ref={setRef}>
    ...
    <header className="hh-detail-hero">
      ...
      <div className="hh-detail-identity">
        ...
        <div className="hh-detail-summary-row">{match?.pct != null && <strong>{match.pct}% Match</strong>}<span>{home.status}</span>{home.reaction === 'love' && <span><Heart size={13} fill="currentColor" /> Favorite</span>}</div>
    ...
    {commuteDestinations.length > 0 && <Section eyebrow="Location & commute" title="Your everyday routes"><div className="hh-detail-commutes">{commuteDestinations.map((destination) => { const state = getState(destination); return <div key={destination.id}><span>{destination.label}</span><strong>{state.status === 'ok' ? `${state.minutes} min` : state.status === 'loading' || state.status === 'idle' ? 'Calculating…' : 'Not available yet'}</strong></div>; })}</div></Section>}

    {match && <Section eyebrow="How it fits your search" title={match.pct == null ? 'More will come into focus' : `Why this home is a ${match.pct}% Match for you`}><div className="hh-detail-match-groups">{['must', 'important', 'nice'].map((tier) => { const rows = match.allSelected.filter((item) => item.tier === tier); return rows.length ? <div key={tier}><h3>{TIER_LABELS[tier]}</h3>{rows.map((item) => { const symbol = !item.evaluated ? (item.objective ? '?' : '—') : item.met ? '✓' : '✕'; const detail = !item.evaluated ? (item.objective ? 'Needs more information' : 'After your tour') : item.detail; return <div className={`hh-detail-criterion ${!item.evaluated ? 'unknown' : item.met ? 'met' : 'missed'}`} key={item.key}><b>{symbol}</b><span><strong>{item.key.includes(':') ? criterionDisplayLabel(item.key.split(':')[0], item.label) : item.label}</strong><small>{detail}</small></span></div>; })}</div> : null; })}</div></Section>}

    <Section eyebrow="Your take" title="Your relationship with this home"><div className="hh-detail-actions"><button className="hh-btn" onClick={() => savePersonal({ status: home.status === 'Want to Tour' ? 'Saved' : 'Want to Tour' })}><Footprints size={15} />{home.status === 'Want to Tour' ? 'On your Want to Tour list' : 'Want to tour'}</button><button className="hh-btn hh-btn-ghost" onClick={() => savePersonal({ reaction: home.reaction === 'love' ? null : 'love' })}><Heart size={15} fill={home.reaction === 'love' ? 'currentColor' : 'none'} />{home.reaction === 'love' ? 'Favorited' : 'Favorite'}</button><button className="hh-btn hh-btn-ghost" onClick={() => savePersonal({ status: isArchivedStatus(home.status) ? (overall ? 'Toured' : 'Saved') : 'Archived' })}>{isArchivedStatus(home.status) ? <><RotateCcw size={15} /> Restore</> : 'Archive'}</button></div>{overall > 0 || experiential.length > 0 ? <div className="hh-detail-evaluation">{overall > 0 && <div><h3>Overall feeling</h3><Stars value={overall} /></div>}{experiential.length > 0 && <div><h3>What you experienced</h3>{experiential.map((item) => <p key={item.key}><span>{criterionDisplayLabel(item.key.split(':')[0], item.label)}</span><strong className={item.met ? 'liked' : 'disliked'}>{item.met ? 'Liked' : "Didn't like"}</strong></p>)}</div>}<button className="hh-detail-text-button" onClick={() => setReflecting(true)}>Edit my thoughts</button></div> : <div className="hh-detail-after-tour"><strong>After your tour</strong><p>Come back to record how the home actually felt.</p><button className="hh-detail-text-button" onClick={() => setReflecting(true)}>Record your take</button></div>}</Section>

    {coBuyerPerspective && (coBuyerPerspective.match || coBuyerPerspective.overallFeeling > 0 || coBuyerPerspective.differentTakes?.length) && <Section eyebrow="Co-buyer perspective" title="Another view, kept distinct"><div className="hh-detail-cobuyer">{coBuyerPerspective.match?.pct != null && <strong>{coBuyerPerspective.match.pct}% Match</strong>}{coBuyerPerspective.overallFeeling > 0 && <span><Stars value={coBuyerPerspective.overallFeeling} /> Overall feeling</span>}</div>{coBuyerPerspective.differentTakes?.length > 0 && <div className="hh-detail-differences"><h3>Different takes</h3>{coBuyerPerspective.differentTakes.map((take) => { const category = take.key?.split(':')[0]; return <p key={take.key}><strong>{criterionDisplayLabel(category, take.label)}</strong><span>{take.youLiked ? 'You liked it' : "You didn't like it"} · {take.coBuyerLiked ? 'Co-Buyer did' : "Co-Buyer didn't"}</span></p>; })}</div>}</Section>}

    <Section eyebrow="Property notes" title="What you want to remember">{!notesOpen ? <><div className="hh-detail-notes">{parseCommaList(home.pros).length > 0 && <div><h3>Pros</h3>{parseCommaList(home.pros).map((x) => <p key={x}>+ {x}</p>)}</div>}{parseCommaList(home.cons).length > 0 && <div><h3>Cons</h3>{parseCommaList(home.cons).map((x) => <p key={x}>− {x}</p>)}</div>}{home.notes && <div className="wide"><h3>Notes</h3><p>{home.notes}</p></div>}</div><button className="hh-detail-text-button" onClick={() => setNotesOpen(true)}>{home.pros || home.cons || home.notes ? 'Edit property notes' : 'Add pros, cons, or a note'}</button></> : <div className="hh-detail-notes-form"><label>Pros<input className="hh-input" value={thoughts.pros} onChange={(e) => setThoughts({ ...thoughts, pros: e.target.value })} placeholder="Great kitchen, quiet street" /></label><label>Cons<input className="hh-input" value={thoughts.cons} onChange={(e) => setThoughts({ ...thoughts, cons: e.target.value })} placeholder="Busy road" /></label><label className="wide">Notes<textarea className="hh-textarea" value={thoughts.notes} onChange={(e) => setThoughts({ ...thoughts, notes: e.target.value })} /></label><div className="wide hh-detail-form-actions"><button className="hh-btn hh-btn-ghost" onClick={() => setNotesOpen(false)}>Cancel</button><button className="hh-btn" disabled={saving} onClick={saveThoughts}>{saving ? 'Saving…' : 'Save notes'}</button></div></div>}</Section>
