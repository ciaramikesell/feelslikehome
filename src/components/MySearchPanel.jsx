...
function WhatMattersCard({ categories, priorities, patch, destinations, setDestinations }) {
      ...
      {!editOpen ? (
        ...
        <div>
          ...
          <div style={{ marginTop: 24, paddingTop: 20, borderTop: '1px solid var(--line)' }}>
            <CommuteDestinations destinations={destinations} onChange={setDestinations} />
          </div>
          <button type="button" className="hh-btn hh-btn-ghost" style={{ fontSize: 11.5, padding: '4px 10px', marginTop: 16 }} onClick={() => setEditOpen(false)}>
            Done
          </button>
...
export default function MySearchPanel({ search, userId, isOwner, participantCount, memberUserId, initialPriorities, initialCommuteDestinations = [] }) {
  ...
  const [destinations, setDestinations] = useState(initialCommuteDestinations);
  ...
  return (
    <div style={{ display: 'grid', gap: 16, maxWidth: 760 }}>
      ...
      <WhatMattersCard categories={categories} priorities={p} patch={patch} destinations={destinations} setDestinations={setDestinations} />
