...
export default function RealtorLanding({ isAuthenticated = false }) {
  const startHref = isAuthenticated ? '/realtor' : '/auth/sign-up?intent=realtor';
  ...
  return <div className="rl-root">
    ...
    <header className="rl-header">
      ...
      <details className="rl-mobile-menu"><summary aria-label="Open navigation"><Menu size={20} /><span>Menu</span></summary><nav aria-label="Mobile Realtor page navigation"><Link href="/">For buyers</Link><a href="#how-it-works">How it works</a><Link href="/auth/sign-in?redirect=/realtor">Sign in</Link><Link className="rl-button is-small" href={startHref}>{isAuthenticated ? 'Open workspace' : 'Get started'}</Link></nav></details>
      {authMode && <LandingAuthPopover mode={authMode} onModeChange={setAuthMode} onClose={closeAuth} returnFocusRef={authTriggerRef} isRealtorEntry redirectTo="/realtor" />}
    </header>

    <main id="main">
      <section className="rl-hero">
        <div className="rl-hero-copy"><p className="rl-eyebrow">For real estate professionals</p><h1>Know what your buyers mean when they say, <em>“It just doesn’t feel right.”</em></h1><p className="rl-lede">Feels Like Home gives you a clearer view of what each client actually values — so you can suggest better-fit homes, understand their tradeoffs, and keep the search moving.</p><div className="rl-actions"><Link className="rl-button" href={startHref}>Start helping a buyer <ArrowRight size={17} aria-hidden="true" /></Link><Link className="rl-text-link" href="/auth/sign-in?redirect=/realtor">Already use FLH? Sign in</Link></div></div>
