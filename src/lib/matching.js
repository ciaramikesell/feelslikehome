...
export function selectHomeCardCriteria(match, mustLimit = 5) {
  ...
  // The compact card preview is intentionally reserved for concrete home
  // features. Baseline search parameters (budget, beds, property type, etc.)
  // remain in `allSelected` and therefore in Match scoring; they simply do not
  // consume this small, glanceable list.
  const isVisibleMustHave = (criterion) => {
    const category = criterion.key?.split(':', 1)[0];
    return criterion.tier === 'must' && (category === 'features' || category === 'exterior');
  };
